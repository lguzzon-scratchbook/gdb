...
process_js_file() {
    ...
    if ! droid exec --skip-permissions-unsafe --model "$DROID_MODEL" \
        "Analyze @{$file} and @${summary_file} to systematically refactor the codebase by creating a comprehensive task list of all functions, then methodically process each function to rename it's name and owned variables with meaningful, descriptive names that improve code readability and maintainability maintaining the code correctness changing all the reference too. For each function processed, document the name and variable renaming in ${summary_file} with before/after mappings, rationale for naming choices, and function context. Continue this iterative process until all functions have been refactored with meaningful names and variable names, ensuring consistency across the codebase and maintaining a complete audit trail of all changes made during the refactoring process. EDIT and READ ONLY $file and ${summary_file} NO OTHER FILES."; then
