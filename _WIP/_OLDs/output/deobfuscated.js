...
var _validateHexCharacter = Array.from(
  ...
  (_formatToHexString, convertToHexString) =>
...
class DataProcessor extends BaseClass {
  ...
  update(__dataBuffer) {
    ...
    const { view: _dataView, buffer: ___dataBuffer, blockLen: blockSize } = this
    const G = __dataBuffer.length
    for (let ___currentIndex = 0; ___currentIndex < G; ) {
      const availableSpace = Math.min(blockSize - this.pos, G - ___currentIndex)
...
var HashComputation = createHashFromHexString(() => new DataProcessorExtended())
...
function processHashValidation(ValidateHashInputs) {
  const validatedOptions = validateFieldAndInitializeOptions(ValidateHashInputs)
  ...
  return (calculateHashPowerResults, resultingHashValue) => {
    ...
    let N = calculateHashPowerResults.mul(calculatedHashPower, calculatedQValue)
...
function calculateSquareRootAndValidate(processHashData) {
  ...
  const exponentiatedValue = fieldValidatorAndInitializer.pow(
    currentExponent,
...
function _calculateLegendreSymbol(isBigIntInRange, hashMap) {
  ...
  const legendreSymbolResult = isBigIntInRange.pow(hashMap, legendreSymbolValue)
...
function calculateExponentialSum(
  ...
  const maxChunksPerExponentiation =
    Math.floor((calculateBigIntExponentiation.BITS - 1) / chunkSize) * chunkSize
...
function validateAndConfigureCurveParameters(
  ...
  for (const curveParameter of ['p', 'n', 'h']) {
  ...
  for (const curveParameterKey of curveParameters) {
...
class ScalarMultiplier {
  ...
  hasCache(____modularExponentiationResult) {
    return getModularExponentiationResult(____modularExponentiationResult) !== 1
  }
}
...
function mergeCalculationsWithDefaults(mathUtil, calculateHalf) {
  ...
  for (const key of Object.keys(calculateHalf)) {
...
function processPrivateKeyHash(_hashOutputProcessor, handleHashComputation) {
  ...
  if (typeof handleHashComputation === 'bigint') {
    ...
    const G = validateAndConvertHashInput('private key', handleHashComputation)
    try {
...
function validateCurveAndKey(validateHashAccumulator, emptyObject = {}) {
  ...
  const weierstrassCurve = curveAndKeyValidationResult.CURVE
  ...
  function generateCompressedCoordinates(
    ...
    if (_validateHashAccumulator) {
      N()
      const isAffineYCoordinateEven = !WeierstrassField.isOdd(affineYCoordinate)
  ...
  class WeierstrassPoint {
    ...
    double() {
      ...
      const pointYSquared = WeierstrassField.mul(pointY, pointY)
    ...
    multiplyUnsafe(scalarMultiplier) {
      ...
      if (scalarMultiplier === modularExponentiationFunction || this.is0()) {
        return WeierstrassPoint.ZERO
      }
      ...
      if (endoElement) {
        ...
        return getFieldProperties.unsafe(this, scalarMultiplier)
      }
    }
    ...
    static fromPrivateKey(privateKey) {
      return WeierstrassPoint.BASE.multiply(
        processPrivateKeyHash(WeierstrassFieldNormalizationFunction, privateKey)
      )
    }
  }
...
function __validateHashAccumulator(isHashAccumulatorValid, _emptyObject = {}) {
  ...
  function _____validateHashAccumulator(hashAccumulatorValidation) {
    try {
      return !!processPrivateKeyHash(___hashFunction, hashAccumulatorValidation)
  ...
  function N(bytesConverterFunction) {
    ...
    const { secretKey, publicKey, publicKeyUncompressed } = __hashAccumulator
...
function initializeCryptographicParameters(
  ...
  class _Signature {
    ...
    static fromBytes(_signatureBytes, B = _initializeCryptographicParameters) {
      ...
      const rSignatureByte = _signatureBytes.subarray(0, orderStructureByteSize)
    ...
    static fromHex(__hexString, B) {
      return _Signature.fromBytes(convertHexStringToUint8Array(__hexString), B)
    }
    ...
    normalizeS() {
      if (this.hasHighS()) {
        return new _Signature(this.r, orderStructure.neg(this.s), this.recovery)
      } else {
        return this
      }
    }
...
function initializeCurveOptions(processScalar) {
  ...
  const allowedPrivateKeyLengthsArray = processScalar.allowedPrivateKeyLengths
    ? Array.from(
        new Set(
  ...
  _tlv: {
    ...
    decode(decodeTlv, tlvData) {
      ...
      if (!isLongFormat) {
        ...
        for (const N of lengthValueBytes) {
  ...
  _int: {
    encode(_inputValue) {
      ...
      if (_inputValue < modularExponentiationFunction) {
        throw new HexEncodingError('integer: negative integers are not allowed')
      }
...
var validateAndConvertMultiplier = (calculateYCoordinateWithHash) =>
  (_calculateYCoordinate[calculateYCoordinateWithHash] ??= crypto.subtle
    ...
    .then((convertUint8ArrayToBase36String) => {
      ...
      let base36String = ''
      for (const G of uint8ArrayToBase36String) {
...
var isExponentiationValid = [
  ...calculateIntermediateResult(
    3,
    (_stunServerURL, stunServerSuffix) =>
...
var ___validateSignature = (
  initializeWebRTC,
  { rtcConfig, rtcPolyfill, turnConfig }
) => {
...
var curveOptions = (
  ...
  const peerEvent = (bufferLowTimeoutPromise) =>
    new Promise((onBufferLowTimeout, handleBufferLowTimeoutError) => {
      const bufferLowTimeoutHandle = setTimeout(() => {
  ...
  const peerId = (processPeerConnection) => {
    ...
    const byteArrayFromCoordinates = new Uint8Array(processScalarWrapper).map(
      (_getYCoordinate, indexLookup) => yCoordinateOutput[indexLookup] || 0
    )
    ...
    const trackPeerConnection = async (
      ...
      const processDataAndCalculate = calculateIntermediateResult(
        ...
        (_calculateCurveData, currentSegmentIndex) => {
          const isLastSegment =
  ...
  const peerEventDispatcher = async () => {
    ...
    for (const [_________peerId, __peerConnection] of peerMap) {
      try {
  ...
  callbackFunction('message', (_createPeerEvent, eventData) =>
    _peerEvent('', eventData)
  )
  fetchPeerById('message', (_removePeerData, __________peerId) => {
  ...
  ___registerPeerEventCallback(
    'message',
    (_registerEventCallbackForPeer, eventCallbackFunction) =>
  ...
  return {
    ...
    removeStream: (streamToRemove, streamRemover) =>
      fetchPeerData(
        streamRemover,
        (_handleStreamRemoval, removeStreamFromHandler) =>
    ...
    removeTrack: (_trackToRemove, __peerData) =>
      fetchPeerData(__peerData, (_removeTrackFunction, trackRemovalFunction) =>
        ...
        if (sessionOffer) {
          ...
          try {
            processHandler.signal(
              await _______________calculateModularExponentiation(sessionAnswer)
            )
          } catch {
            signatureProcessingFunction?.({
...
var signatureValidatorFunction = (valueOrDefaultShuffleArray) =>
  (shuffleArray[valueOrDefaultShuffleArray] ??=
    ...
    20000)
var getRandomIndex = (trimTrailingSlash) => trimTrailingSlash.replace(/\/$/, '')
...
var signalData = ________calculateModularExponentiation(generateRandomizedValue)
export {
