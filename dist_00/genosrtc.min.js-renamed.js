...
function $0($, ...J) {
  if (!findResult($)) throw new Error('Uint8Array expected')
...
function S0($ = 32) {
  if (result && typeof result.getRandomValues === 'function')
    return result.getRandomValues(new Uint8Array($));
  if (result && typeof result.randomBytes === 'function')
    return Uint8Array.from(result.randomBytes($));
...
function m($, J, q) {
  ...
  else if (findResult(J)) Q = Uint8Array.from(J)
...
function uQ($, J, q = {}) {
  ...
  function S(M) {
    let B = void 0
    const Y = typeof M === 'string' || findResult(M),
      D =
        !Y &&
        M !== null &&
        typeof M === 'object' &&
        ...
        C0 = (S) => async (W, O, M) => {
          ...
          if (C) {
            ...
            ;(j[D] ||= []),
              (j[D][S] = T),
              T.setHandlers({ connect: () => c(T, D), close: () => s(T, D) }),
              M(R, e({ peerId: key, offer: g, peer: T }))
          }
        },
...
var l0 = {},
  ...
  currentValue = jQ({
    ...
    announce: async ($, J) => {
      if (J$.has(P8($.url))) return
      $.send(await DQ(J, e({ peerId: key })))
    }
  }),
  nextElement = KQ(l0)
export { key as selfId, currentValue as join, nextElement as getRelaySockets }
