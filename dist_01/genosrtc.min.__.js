...
var j$ = (() =>
	...
	OQ = Array.from({ length: 256 }, (_$, J) => J.toString(16).padStart(2, "0")),
	X0 = { _0: 48, _9: 57, A: 65, F: 70, a: 97, f: 102 };
class r0 {}
function RQ($, J, q, Q) {
...
class E8 extends r0 {
	...
	digestInto($) {
		...
		let { pos: z } = this;
		if (((J[z++] = 128), I0(this.buffer.subarray(z)), this.padOffset > Q - z))
			this.process(q, 0), (z = 0);
	...
	_cloneInto($) {
		...
		if (
			(($.destroyed = z), ($.finished = G), ($.length = Q), ($.pos = U), Q % J)
		)
			$.buffer.set(q);
		return $;
	}
...
class K$ extends E8 {
	...
	get() {
		const { A: $, B: J, C: q, D: Q, E: G, F: z, G: U, H: X } = this;
	...
	process($, J) {
		...
		let { A: q, B: Q, C: G, D: z, E: U, F: X, G: Z, H: V } = this;
		for (let N = 0; N < 64; N++) {
...
class f8 extends r0 {
	constructor($, J) {
		...
		const q = o0(J);
		if (((this.iHash = $.create()), typeof this.iHash.update !== "function"))
...
function n0($, J, q = {}) {
	...
	Object.entries(J).forEach(([G, z]) => Q(G, z, !1)),
		Object.entries(q).forEach(([G, z]) => Q(G, z, !0));
}
function m8($) {
	const J = new WeakMap();
	return (q, ...Q) => {
...
function W8($, J) {
	...
	const q = J !== void 0 ? J : $.toString(2).length,
		Q = Math.ceil(q / 8);
	return { nBitLength: q, nByteLength: Q };
}
function O0($, J, q = !1, Q = {}) {
	...
	const L = Object.freeze({
		...
		fromBytes: (j, K = !0) => {
			...
			if (j.length !== V)
				throw new Error(
					`Field.fromBytes: expected ${V} bytes, got ${j.length}`,
				);
			let H = q ? y8(j) : M0(j);
			if (U) H = p(H, $);
			if (!K) {
...
var k$ = ($, J) => {
	...
	c8 = ($, J) => {
		...
		const q = Math.ceil(J / $) + 1,
			...
			z = H0($),
			U = BigInt($);
		return { windows: q, windowSize: Q, mask: z, maxNumber: G, shiftBy: U };
	},
	A$ = ($, J, q) => {
		...
		const V = J * Q,
			...
			j = X < 0,
			K = J % 2 !== 0;
		return { nextN: Z, offset: N, isZero: L, isNeg: j, isNegF: K, offsetF: V };
	},
	fQ = ($, J) => {
...
function E$($, J, q, Q) {
	...
	for (let K = L; K >= 0; K -= Z) {
		...
		for (let A = N.length - 1, E = U; A > 0; A--)
			(E = E.add(N[A])), (H = H.add(E));
		if (((j = j.add(H)), K !== 0)) for (let A = 0; A < Z; A++) j = j.double();
	}
	return j;
}
var I$ = ($, J, q) => {
...
function f$($, J, q = {}, Q) {
	...
	return (J = Object.freeze(Object.assign({}, J))), { CURVE: J, Fp: G, Fn: z };
} /*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) */
var g0 = BigInt(0),
	E0 = BigInt(1),
	d8 = new WeakMap(),
...
function bQ($, J, q) {
	...
	return { k1neg: L, k1: V, k2neg: j, k2: N };
}
var i8 = ($) => {
	...
	p8 = ($, J) => {
		const q = {};
		for (const Q of Object.keys(J)) q[Q] = $[Q] === void 0 ? J[Q] : $[Q];
		if ((p0(q.lowS, "lowS"), p0(q.prehash, "prehash"), q.format !== void 0))
			i8(q.format);
		return q;
	};
function R0($, J) {
...
function gQ($, J = {}) {
	...
	const { endo: Z } = J;
	if (Z) {
		if (!Q.is0(z.a) || typeof Z.beta !== "bigint" || !Array.isArray(Z.basises))
			throw new Error(
	...
	const s = m8((W, O) => {
			const { X: M, Y: B, Z: Y } = W;
			if (Q.eql(Y, Q.ONE)) return { x: M, y: B };
			const D = W.is0();
			if (O == null) O = D ? Q.ONE : Q.inv(Y);
			const C = Q.mul(M, O),
				x = Q.mul(B, O),
				T = Q.mul(Y, O);
			if (D) return { x: Q.ZERO, y: Q.ZERO };
	...
	class I {
		...
		equals(W) {
			...
			const { X: O, Y: M, Z: B } = this,
				{ X: Y, Y: D, Z: C } = W,
				x = Q.eql(Q.mul(O, C), Q.mul(Y, B)),
				T = Q.eql(Q.mul(M, C), Q.mul(D, B));
			return x && T;
		}
		negate() {
...
function yQ($, J = {}) {
	const { Fn: q } = $,
		Q = J.randomBytes || S0,
		G = Object.assign(y$($.Fp, q), { seed: l8(q.ORDER) });
	function z(K) {
		try {
			return !!R0(q, K);
		} catch (_H) {
	...
	function V(K) {
		const H = X(K);
		return { secretKey: H, publicKey: Z(H) };
	}
	function N(K) {
...
function uQ($, J, q = {}) {
	...
	class _ {
		constructor(M, B, Y) {
			if (((this.r = v("r", M)), (this.s = v("s", B)), Y != null))
				this.recovery = Y;
			Object.freeze(this);
		}
		...
		normalizeS() {
			return this.hasHighS()
				? new _(this.r, U.neg(this.s), this.recovery)
				: this;
		}
		toDERRawBytes() {
	...
	function G0(M, B) {
		return (
			x0(M, void 0, "message"), B ? x0(J(M), void 0, "prehashed message") : M
		);
	}
	function I(M, B, Y) {
	...
	function W(M, B, Y, D = {}) {
		...
		if (((Y = m("publicKey", Y)), (B = G0(m("message", B), x)), "strict" in D))
		...
		try {
			...
			const { r: k, s: w } = R,
				...
				b = U.create(P * f),
				y = U.create(k * f),
				u = $.BASE.multiplyUnsafe(b).add(g.multiplyUnsafe(y));
			if (u.is0()) return !1;
	...
	function O(M, B, Y = {}) {
		...
		return (
			(B = G0(B, D)), _.fromBytes(M, "recovered").recoverPublicKey(B).toBytes()
		);
	}
	return Object.freeze({
		keygen: V,
...
var mQ = ($) => {
	...
	FQ = ($) => {
		...
		return { CURVE: J, curveOpts: q, hash: $.hash, ecdsaOpts: Q };
	};
var hQ = ($, J) => {
	const q = J.Point;
...
var dQ = ($) => {
	...
	n8 = ($) => {
		const { Fn: J, BASE: q } = m0,
			Q = R0(J, $),
			G = q.multiply(Q);
		return { scalar: t8(G.y) ? Q : J.neg(Q), bytes: s8(G) };
	},
	d$ = ($) => {
		const J = T8;
		...
		let G = J.sqrt(Q);
		if (!t8(G)) G = J.neg(G);
		const z = m0.fromAffine({ x: $, y: G });
		return z.assertValidity(), z;
	},
	o$ = (...$) => m0.Fn.create(t0(B8("BIP0340/challenge", ...$))),
	l$ = ($) => n8($).bytes,
	oQ = ($, J, q = S0(32)) => {
		const { Fn: Q } = m0,
			G = m("message", $),
			{ bytes: z, scalar: U } = n8(J),
			X = m("auxRand", q, 32),
			Z = Q.toBytes(U ^ t0(B8("BIP0340/aux", X))),
			V = B8("BIP0340/nonce", Z, z, G),
			{ bytes: N, scalar: L } = n8(V),
			j = o$(N, z, G),
			K = new Uint8Array(64);
		if ((K.set(N, 0), K.set(Q.toBytes(Q.create(L + j * U)), 32), !r$(K, G, z)))
...
var u0 = {
	...
	a8 = BigInt(2),
	T8 = O0(u0.p, { sqrt: dQ }),
	c$ = m$({ ...u0, Fp: T8, lowS: !0, endo: lQ }, K8),
	h$ = {},
	...
	x8 = (() => {
		...
		function Q(G) {
			const z = q(G);
			return { secretKey: z, publicKey: l$(z) };
		}
		return {
			keygen: Q,
			getPublicKey: l$,
...
var e8 = "AES-GCM",
	...
	e0 = ($) =>
		(iQ[$] ??= crypto.subtle.digest("SHA-1", j0($)).then((J) => {
			const q = new Uint8Array(J);
			let Q = "";
			for (const G of q) Q += G.toString(36);
			return Q;
		})),
	a$ = async ($, J, q) => {
		const Q = await crypto.subtle.digest("SHA-256", j0(`${$}:${J}:${q}`));
		return crypto.subtle.importKey("raw", Q, { name: e8 }, !1, [
			"encrypt",
			"decrypt",
		]);
	},
	n$ = async ($, J) => {
		const q = crypto.getRandomValues(new Uint8Array(12)),
			Q = await $,
			G = await crypto.subtle.encrypt({ name: e8, iv: q }, Q, j0(J));
	...
	s$ = async ($, J) => {
		...
		const G = Uint8Array.from(q.split(i$), Number),
			z = await $,
			U = await crypto.subtle.decrypt({ name: e8, iv: G }, z, nQ(Q));
		return f0(U);
	};
var a = "GenosRTC",
	F0 = ($, J) => Array.from({ length: $ }, J);
var $8 = ($) =>
		Array.from(
	...
	{ entries: e$, fromEntries: R8, keys: J1 } = Object,
	$Q = () => {},
...
var sQ = 5000,
	...
	L8 = ($, { rtcConfig: J, rtcPolyfill: q, turnConfig: Q }) => {
		...
		const V = (j) =>
				Object.assign(j, {
					...
					onclose: () => z.close?.(),
					onerror: (K) =>
						!K?.error?.message?.includes("User-Initiated Abort") &&
						z.error?.(K),
				}),
			N = () =>
				Promise.race([
				...
				try {
					if (j.type === qQ) {
						if (U || (G.signalingState !== "stable" && !X)) {
							if ($) return;
							await V0([
								G.setLocalDescription({ type: "rollback" }),
