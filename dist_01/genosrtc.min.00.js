...
function isUint8Array(arr) {
  return arr instanceof Uint8Array || ArrayBuffer.isView(arr) && arr.constructor.name === "Uint8Array"
}
...
function assertBytes(bytes, ...expectedLengths) {
  ...
  if (expectedLengths.length > 0 && !expectedLengths.includes(bytes.length)) {
    throw new Error("Uint8Array expected of length " + expectedLengths + ", got length=" + bytes.length)
  }
}
...
const isNativeHex = (() => typeof Uint8Array.from([]).toHex === "function" && typeof Uint8Array.fromHex === "function")()
...
function hexToBytes(hex) {
  ...
  if (hexLen % 2) {
    throw new Error("hex string expected, got unpadded hex of length " + hexLen)
  }
  ...
  for (let i = 0, j = 0; i < bytesLen; i++, j += 2) {
    ...
    if (high === undefined || low === undefined) {
      ...
      throw new Error("hex string expected, got non-hex character \"" + char + "\" at index " + j)
    }
...
class SHA256 extends SHA2 {
  ...
  process(dataView, dataOffset) {
    ...
    for (let i = 0; i < 64; i++) {
      ...
      const T1 = h + S1 + chi(e, f, g) + SHA256_ROUND_CONSTANTS[i] + SHA256_W[i] | 0
      const T2 = (rotr32(a, 2) ^ rotr32(a, 13) ^ rotr32(a, 22)) + maj(a, b, c) | 0
      h = g
      g = f
      f = e
      e = d + T1 | 0
...
class HMAC extends Hash {
  constructor(hash, key) {
    ...
    block.set(keyBytes.length > blockLen ? hash.create().update(keyBytes).digest() : keyBytes)
...
function validateObject(obj, required, optional = {}) {
  ...
  function check(key, type, isOptional) {
    ...
    if (valType !== type || val === null) {
      throw new Error(`param "${key}" is invalid: expected ${type}, got ${valType}`)
    }
  }
...
function invertBatch(field, nums, isNonZero = false) {
  const scratch = new Array(nums.length).fill(isNonZero ? field.ZERO : undefined)
...
function createField(order, opts, isLE = false, opts2 = {}) {
  ...
  let bitsOpt = undefined
  let sqrtOpt = undefined
  let modFromBytesOpt = false
  let allowedLengthsOpt = undefined
  ...
  const field = Object.freeze({
    ...
    toBytes: n => isLE ? numberToBytesLE(n, BYTES) : numberToBytesBE(n, BYTES),
    fromBytes: (bytes, isSafe = true) => {
      if (allowedLengthsOpt) {
        if (!allowedLengthsOpt.includes(bytes.length) || bytes.length > BYTES) {
          throw new Error("Field.fromBytes: expected " + allowedLengthsOpt + " bytes, got " + bytes.length)
        }
...
function normalizeZ(curve, points) {
  const Zs = invertBatch(curve.Fp, points.map(p => p.Z))
...
function validateWnafWindow(w, precomputedBits) {
  if (!Number.isSafeInteger(w) || w <= 0 || w > precomputedBits) {
    throw new Error("invalid window size, expected [1.." + precomputedBits + "], got W=" + w)
  }
}
...
function validateCurve(curveType, curve, opts = {}, defaultIsLE = curveType === "edwards") {
  if (!curve || typeof curve !== "object") {
    throw new Error(`expected valid ${curveType} CURVE object`)
  }
  for (let key of ["p", "n", "h"]) {
    const val = curve[key]
  ...
  const weierstrassParams = ["Gx", "Gy", "a", curveType === "weierstrass" ? "b" : "d"]
  for (let key of weierstrassParams) {
...
class WNAF {
  ...
  unsafe(point, scalar, transform, initial) {
    ...
    return this.wNAFUnsafe(W, this.getPrecomputes(point, W, transform), scalar, initial)
  }
  createCache(point, W) {
...
function normalizePrivateKey(field, key) {
  ...
  if (typeof key === "bigint") {
    ...
    let bytesKey = ensureBytes("private key", key)
    try {
      numKey = field.fromBytes(bytesKey)
    } catch (e) {
      throw new Error(`invalid private key: expected ui8a of size ${len}, got ${typeof key}`)
    }
  }
...
function createCurve(curveConfig, opts) {
  ...
  function pointToBytes(curve, point, isCompressed) {
    const {
      x,
      y
    } = point.toAffine()
  ...
  class ProjectivePoint {
    ...
    static fromBytes(bytes) {
      const p = ProjectivePoint.fromAffine(fromBytesFn(assertBytes(bytes, undefined, "point")))
      p.assertValidity()
      return p
    }
    static fromHex(hex) {
    ...
    multiply(scalar) {
      ...
      let point
      let fake
      const getPrecomputes = (W) => wnaf.cached(this, W, c => normalizeZ(ProjectivePoint, c))
      if (endo) {
        const {
          k1neg,
          k1,
    ...
    multiplyUnsafe(scalar) {
      ...
      if (endo) {
        ...
        const {
          p1: u1,
          p2: u2
        } = unsafeLadder(ProjectivePoint, P, k1, k2)
        return addEndo(endo.beta, u1, u2, k1neg, k2neg)
      } else {
        return wnaf.unsafe(P, scalar)
      }
    }
  ...
  const nBits = Fn.BITS
  const wnaf = new WNAF(ProjectivePoint, opts.endo ? Math.ceil(nBits / 2) : nBits)
...
function createCiphers(curve, opts = {}) {
  ...
  function randomSecretKey(seed = randomBytes(lengths.seed)) {
    return hashToPrivateBytes(assertBytes(seed, lengths.seed, "seed"), Fn.ORDER)
  }
  function getPublicKey(key, isCompressed = true) {
    return curve.BASE.multiply(normalizePrivateKey(Fn, key)).toBytes(isCompressed)
  }
...
function createEcdsa(curve, hash, opts = {}) {
  ...
  function assertValidSignature(part, val) {
    if (!Fn.isValidNot0(val)) {
      throw new Error(`invalid signature ${part}: out of range 1..Point.Fn.ORDER`)
    }
    return val
  }
  function assertSignature(data, format) {
    ...
    const recLen = format === "compact" ? sigLen : format === "recovered" ? sigLen + 1 : undefined
  ...
  class Signature {
    ...
    recoverPublicKey(msgHash) {
      ...
      const R = curve.fromBytes(concatBytes(getPointCompress((recovery & 1) === 0), Rbytes))
      const r_inv = Fn.inv(r)
      const z =
        bits2int_modN(ensureBytes("msgHash", msgHash))
  ...
  function parseSignature(signature) {
    let sig = undefined
    const isStringOrBytes = typeof signature === "string" || isUint8Array(signature)
    const isSigObject = !isStringOrBytes && signature !== null && typeof signature === "object" && typeof signature.r === "bigint" && typeof signature.s === "bigint"
  ...
  function verify(signature, msgHash, pub, opts = {}) {
    ...
    const sig = format === undefined ? parseSignature(signature) : Signature.fromBytes(ensureBytes("sig", signature), format)
    if (sig === false) {
      return false
    }
    try {
...
function getCurveConfig(curve) {
  ...
  const Fp = curve.Fp
  let allowedPrivateKeyLengths = curve.allowedPrivateKeyLengths ? Array.from(new Set(curve.allowedPrivateKeyLengths.map(l => Math.ceil(l / 2)))) : undefined
...
function wrapEcdsa(curve, ecdsa) {
  ...
  return Object.assign({}, ecdsa, {
    ...
    CURVE: Object.assign({}, curve, getCurveParams(Point.Fn.ORDER, Point.Fn.BITS))
  })
}
...
function createCurveWithHash(curveConfig, defaultHash) {
  const creator = (hash) => createEcdsaCiphers({
    ...curveConfig,
    hash: hash
  })
  return {
...
function challenge(...messages) {
  return secp256k1Point.Fn.create(bytesToNumberBE(taggedHash("BIP0340/challenge", ...messages)))
}
...
const bytesToHex = (arr) => Array.from(arr, (b) => b.toString(16).padStart(2, "0")).join("")
const jsonStringify = JSON.stringify
const jsonParse = JSON.parse
const stringToHashNumber = (str, max = Number.MAX_SAFE_INTEGER) => str.split("").reduce((acc, char) => acc + char.charCodeAt(0), 0) % max
...
const sha1 = (s) => sha1Cache[s] ??= crypto.subtle.digest("SHA-1", utf8ToBytes(s)).then((hash) => {
...
const createAesKey = async (password, appId, roomId) => {
  const keyMaterial = await crypto.subtle.digest("SHA-256", utf8ToBytes(`${password}:${appId}:${roomId}`))
...
const decrypt = async (keyPromise, encryptedPayload) => {
  const [ivStr, ciphertextB64] = encryptedPayload.split(ENCRYPT_PAYLOAD_SEPARATOR)
  ...
  const plaintext = await crypto.subtle.decrypt({
    name: AES_GCM_ALGORITHM,
    iv: iv
  }, key, base64ToArrayBuffer(ciphertextB64))
  return bytesToUtf8(plaintext)
}
const LIB_NAME = "GenosRTC"
const initArray = (len, cb) => Array.from({
  length: len
}, cb)
const randomId = (len) => initArray(len, null).map(() => "0123456789AaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZz"[crypto.getRandomValues(new Uint8Array(1))[0] % "0123456789AaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZz".length]).join("")
...
const OFFER = "offer"
const ANSWER = "answer"
const DEFAULT_STUN_SERVERS = [...initArray(3, (el, i) => `stun:stun${i || ""}.l.google.com:19302`), "stun:stun.cloudflare.com:3478"].map((url) => ({
  urls: url
}))
const createPeer = (isInitiator, {
  ...
  let channel
  const initChannel = (ch) => Object.assign(ch, {
    binaryType: "arraybuffer",
    bufferedAmountLowThreshold: 65535,
    onmessage: (e) => handlers.data?.(e.data),
    onopen: () => handlers.connect?.(),
    onclose: () => handlers.close?.(),
    onerror: (e) => !e?.error?.message?.includes("User-Initiated Abort") && handlers.error?.(e)
  })
  const gatherIce = () => Promise.race([new Promise((resolve) => {
    const onIceGatheringStateChange = () => {
      if (pc.iceGatheringState === "complete") {
        pc.removeEventListener(ICE_GATHERING_STATE_CHANGE_EVENT, onIceGatheringStateChange)
        resolve()
      }
    }
    pc.addEventListener(ICE_GATHERING_STATE_CHANGE_EVENT, onIceGatheringStateChange)
  ...
  return {
    ...
    async signal(desc) {
      ...
      try {
        if (desc.type === OFFER) {
          if (makingOffer || pc.signalingState !== "stable" && !ignoringOffer) {
            if (isInitiator) {
              return
            }
            await promiseAll([pc.setLocalDescription({
              type: "rollback"
    ...
    removeStream: (stream) => getSenders().filter((sender) => stream.getTracks().includes(sender.track)).forEach((sender) => pc.removeTrack(sender)),
...
const CHUNK_HEADER_OFFSET_NONCE = CHUNK_HEADER_OFFSET_TYPE + ACTION_TYPE_MAX_BYTES
...
const initLib = (onPeer, onPeerLeave, onLibClose) => {
  ...
  const emit = (name, ...args) => subscribers.get(name)?.forEach((cb) => cb(...args))
  ...
  const waitForBufferlow = (channel) => new Promise((resolve, reject) => {
  ...
  const channel = (type) => {
    ...
    if (typeBytes.byteLength > ACTION_TYPE_MAX_BYTES) {
      throw GenosError(`\u274C "${type}" exceeds ${ACTION_TYPE_MAX_BYTES} bytes. Use a shorter name.`)
    }
    const anameBytes = new Uint8Array(ACTION_TYPE_MAX_BYTES).map((el, i) => typeBytes[i] || 0)
    let nonce = 0
    ...
    const send = async (data, peerIds, meta, onProgress) => {
      ...
      const dataBytes = isBinary ? new Uint8Array(isBlob ? await data.arrayBuffer() : data) : utf8ToBytes(isJson ? jsonStringify(data) : data)
      const metaBytes = meta ? utf8ToBytes(jsonStringify(meta)) : null
      const numChunks = Math.ceil(dataBytes.byteLength / CHUNK_PAYLOAD_MAX_LENGTH) + (meta ? 1 : 0) || 1
      if (numChunks > MAX_CHUNKS) {
        throw GenosError(`Message too large, exceeds max chunks ${MAX_CHUNKS}`)
      }
      const chunks = initArray(numChunks, (el, i) => {
        const isLast = i === numChunks - 1
        const isMeta = meta && i === 0
        const payload = isMeta ? metaBytes : dataBytes.subarray(meta ? (i - 1) * CHUNK_PAYLOAD_MAX_LENGTH : i * CHUNK_PAYLOAD_MAX_LENGTH, meta ? i * CHUNK_PAYLOAD_MAX_LENGTH : (i + 1) * CHUNK_PAYLOAD_MAX_LENGTH)
        ...
        chunk.set([isLast | isMeta << 1 | isBinary << 2 | isJson << 3], CHUNK_HEADER_OFFSET_FLAGS)
        chunk.set([Math.round((i + 1) / numChunks * UINT8_MAX)], CHUNK_HEADER_OFFSET_PROGRESS)
        chunk.set(payload, CHUNK_HEADER_LENGTH)
        return chunk
      })
      nonce = nonce + 1 & UINT8_MAX
      return promiseAll(broadcast(peerIds, async (id, peer) => {
        const {
          channel
        } = peer
        for (let i = 0; i < numChunks; i++) {
          ...
          while (channel.bufferedAmount > channel.bufferedAmountLowThreshold) {
            if (retries++ > SEND_BUFFER_RETRIES) {
              throw GenosError(`${LIB_NAME}: send buffer full, max retries reached for peer ${id}`)
            }
            try {
              await waitForBufferlow(channel)
            } catch (err) {
              console.warn(err.message)
              await new Promise((res) => setTimeout(res, SEND_BUFFER_RETRY_DELAY))
            }
          }
          if (!peers.has(id)) {
            break
          ...
          onProgress?.(chunks[i][CHUNK_HEADER_OFFSET_PROGRESS] / UINT8_MAX, id, meta)
        }
      }))
    }
    actions.set(type, {
  ...
  const handleData = (id, arrayBuffer) => {
    try {
      const bytes = new Uint8Array(arrayBuffer)
      const type = bytesToUtf8(bytes.subarray(CHUNK_HEADER_OFFSET_TYPE, CHUNK_HEADER_OFFSET_NONCE)).replace(/�/g, "")
      if (!actions.has(type)) {
        return console.warn(`${LIB_NAME}: unregistered type (${type})`)
      }
      const [nonce] = bytes.subarray(CHUNK_HEADER_OFFSET_NONCE, CHUNK_HEADER_OFFSET_FLAGS)
      const [flags] = bytes.subarray(CHUNK_HEADER_OFFSET_FLAGS, CHUNK_HEADER_OFFSET_PROGRESS)
      const [progress] = bytes.subarray(CHUNK_HEADER_OFFSET_PROGRESS, CHUNK_HEADER_LENGTH)
      ...
      if (ns[type]?.[nonce]?.chunks?.length > MAX_CHUNKS) {
        console.warn(`${LIB_NAME}: peer ${id} sent too many chunks for nonce ${nonce}, ignoring.`)
        return
      }
      ...
      if (isMeta) {
        try {
          ...
          console.warn(`${LIB_NAME}: failed to parse meta from peer ${id} for type ${type}`)
          return
        }
      } else {
      ...
      const full = new Uint8Array(pending.chunks.reduce((acc, chunk) => acc + chunk.byteLength, 0))
      ...
      if (isBinary) {
        ...
        try {
          ...
          console.warn(`${LIB_NAME}: failed to parse JSON message data from peer ${id} for type ${type}`)
          return
        }
      } else {
  ...
  const {
    send: sendPing,
    on: onPing
  } = channel(makeInternalName("ping"))
  const {
    send: sendPong,
    on: onPong
  } = channel(makeInternalName("pong"))
  const {
    send: sendSignal,
    on: onSignal
  } = channel(makeInternalName("signal"))
  const {
    send: sendStreamMeta,
    on: onStreamMeta
  } = channel(makeInternalName("stream"))
  const {
    send: sendTrackMeta,
    on: onTrackMeta
  } = channel(makeInternalName("track"))
  const {
    send: sendLeave,
    on: onLeave
  } = channel(makeInternalName("leave"))
  onPeer((peer, id) => {
    if (peers.has(id)) {
      return
    }
  ...
  onPing("message", (payload, id) => sendPong("", id))
  onPong("message", (payload, id) => {
    pingDeferreds[id]?.()
    delete pingDeferreds[id]
  })
  ...
  onLeave("message", (payload, id) => cleanupPeer(id))
  if (isBrowser) {
  ...
  return {
    ...
    addStream: (stream, peerIds, meta) => broadcast(peerIds, async (id, peer) => {
      if (meta) {
        await sendStreamMeta(meta, id)
      }
      peer.addStream(stream)
    }),
    removeStream: (stream, peerIds) => broadcast(peerIds, (id, peer) => peer.removeStream(stream)),
    addTrack: (track, stream, peerIds, meta) => broadcast(peerIds, async (id, peer) => {
      if (meta) {
        await sendTrackMeta(meta, id)
      }
      peer.addTrack(track, stream)
    }),
    removeTrack: (track, peerIds) => broadcast(peerIds, (id, peer) => peer.removeTrack(track)),
    replaceTrack: (oldTrack, newTrack, peerIds, meta) => broadcast(peerIds, async (id, peer) => {
      if (meta) {
        await sendTrackMeta(meta, id)
      }
...
const createBroker = ({
  ...
  return (config, roomId, onWarning) => {
    ...
    if (!config || !roomId || !appId && !config.firebaseApp) {
      throw GenosError(!config ? "config required" : !roomId ? "roomId required" : "appId missing")
    }
    let onPeer = noop
    if (!isInitialized) {
    ...
    const onMessage = (dbIndex) => async (topicA, payload, talk) => {
      ...
      if (offer) {
        ...
        try {
          ...
          const [topicB, encryptedAnswer] = await promiseAll([sha1(makeKey(nsKey, otherId)), peer.signal(decryptedOffer).then(encryptSdp)])
          talk(topicB, jsonStringify({
            peerId: selfId,
        ...
        }] = await promiseAll([sha1(makeKey(nsKey, otherId)), peer.offerPromise.then(encryptSdp).then((offer) => ({
          offer
        }))])
        offers[otherId] ||= []
    ...
    const subs = promiseAll(db.map(async (d, i) => subscribe(await d, await selfHash, await selfIdHash, onMessage(i))))
    const timers = []
    subs.then((unsubs) => {
      db.forEach(async (d, i) => {
        const Looper = async () => {
          const timeout = await announce(await d, await selfHash, await selfIdHash)
          timers[i] = setTimeout(Looper, typeof timeout === "number" ? timeout : ANNOUNCE_INTERVAL)
        }
        Looper()
      })
    ...
    nsCache[appId] ||= {}
    const lib = initLib((peer, id) => onPeer = peer, (id) => delete connectedPeers[id], () => lib.onCleanup?.())
...
if (typeof window !== "undefined") {
  const forceReconnect = () => {
    ...
    Object.values(socketCache).forEach((ctl) => {
      if (ctl.socket && ctl.socket.readyState !== WebSocket.OPEN && ctl.socket.readyState !== WebSocket.CONNECTING) {
        ctl.forceReconnect()
      }
    })
  }
...
const getRelaySockets = (socketCache) => () => objectFromEntries(objectEntries(socketCache).map(([url, ctl]) => [url, ctl.socket]))
const getActiveRelays = (config, relays, defaultRedundancy, canShuffle) => (config.relayUrls ?? (canShuffle ? shuffle(relays, stringToHashNumber(config.appId)) : relays)).slice(0, config.relayUrls?.length ?? config.relayRedundancy ?? defaultRedundancy)
...
const createNostrEvent = async (topic, content) => {
  ...
  const payload = utf8ToBytes(jsonStringify([0, event.pubkey, event.created_at, event.kind, event.tags, event.content]))
  const hash = new Uint8Array(await crypto.subtle.digest("SHA-256", payload))
  const sig = schnorr.sign(hash, nostrPrivateKey)
  return jsonStringify([NOSTR_EVENT_KIND, {
    ...event,
    id: bytesToHex(hash),
    sig: bytesToHex(sig)
  }])
...
const join = createBroker({
  init: (config) => {
    const relays = (config?.relayUrls?.length ? config.relayUrls : null) ?? NOSTR_DEFAULT_RELAYS
    return getActiveRelays(config, relays, NOSTR_RELAY_REDUNDANCY, true).map(getNostrSocket).map((socket) => socket.ready.then(() => socket).catch(() => null))
  },
  subscribe: (socket, topicA, topicB, onMessage) => {
    const subIdA = randomId(64)
    const subIdB = randomId(64)
    nostrSubIdToHandler[subIdA] = nostrSubIdToHandler[subIdB] = (topic, content) => onMessage(topic, content, async (destTopic, msg) => {
  ...
  announce: async (socket, topic) => {
    ...
    socket.send(await createNostrEvent(topic, jsonStringify({
      peerId: selfId
    })))
  }
})
...
export {
  selfId,
  join,
  getRelaySockets_ as getRelaySockets
}