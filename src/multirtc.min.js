...
var Z$ = (() =>
  ...
  wQ = Array.from({ length: 256 }, ($, J) => J.toString(16).padStart(2, '0')),
  Y0 = { _0: 48, _9: 57, A: 65, F: 70, a: 97, f: 102 }
class p0 {}
function IQ($, J, q, Q) {
...
class f8 extends p0 {
  ...
  digestInto($) {
    ...
    let { pos: z } = this
    if (((J[z++] = 128), k0(this.buffer.subarray(z)), this.padOffset > Q - z))
...
var E$ = ($, J) => {
    if (!Number.isSafeInteger($) || $ <= 0 || $ > J)
      throw new Error(
        'invalid window size, expected [1..' + J + '], got W=' + $
      )
  },
  o8 = ($, J) => {
    E$($, J)
  ...
  k$ = ($, J, q) => {
    ...
    const V = J * Q,
      ...
      j = Y < 0,
      K = J % 2 !== 0
    return { nextN: X, offset: N, isZero: L, isNeg: j, isNegF: K, offsetF: V }
  },
  gQ = ($, J) => {
...
function _$($, J, q, Q) {
  ...
  for (let K = L; K >= 0; K -= X) {
    ...
    for (let A = N.length - 1, E = U; A > 0; A--)
      (E = E.add(N[A])), (x = x.add(E))
    if (((j = j.add(x)), K !== 0)) for (let A = 0; A < X; A++) j = j.double()
  }
  return j
}
var P$ = ($, J, q) => {
...
var n8 = ($) => {
  ...
  a8 = ($, J) => {
    ...
    if ((i0(q.lowS, 'lowS'), i0(q.prehash, 'prehash'), q.format !== void 0))
      n8(q.format)
    return q
  }
function L0($, J) {
...
function FQ($, J = {}) {
  ...
  const { endo: X } = J
  if (X) {
    if (!Q.is0(z.a) || typeof X.beta !== 'bigint' || !Array.isArray(X.basises))
      throw new Error(
  ...
  const s = h8((W, T) => {
      const { X: M, Y: H, Z } = W
      if (Q.eql(Z, Q.ONE)) return { x: M, y: H }
      const D = W.is0()
  ...
  class I {
    ...
    equals(W) {
      ...
      const { X: T, Y: M, Z: H } = this,
        { X: Z, Y: D, Z: C } = W,
        O = Q.eql(Q.mul(T, C), Q.mul(Z, H)),
        B = Q.eql(Q.mul(M, C), Q.mul(D, H))
      return O && B
    }
    negate() {
...
function hQ($, J = {}) {
  const { Fn: q } = $,
    Q = J.randomBytes || P0,
    G = Object.assign(F$($.Fp, q), { seed: d8(q.ORDER) })
  function z(K) {
    try {
      return !!L0(q, K)
    } catch (x) {
  ...
  function V(K) {
    const x = Y(K)
    return { secretKey: x, publicKey: X(x) }
  }
  function N(K) {
...
function lQ($, J, q = {}) {
  ...
  class b {
    constructor(M, H, Z) {
      if (((this.r = f('r', M)), (this.s = f('s', H)), Z != null))
        this.recovery = Z
      Object.freeze(this)
    }
    ...
    normalizeS() {
      return this.hasHighS()
        ? new b(this.r, U.neg(this.s), this.recovery)
        : this
    }
    toDERRawBytes() {
  ...
  function G0(M, H) {
    return (
      O0(M, void 0, 'message'), H ? O0(J(M), void 0, 'prehashed message') : M
    )
  }
  function I(M, H, Z) {
  ...
  function W(M, H, Z, D = {}) {
    ...
    if (((Z = m('publicKey', Z)), (H = G0(m('message', H), O)), 'strict' in D))
    ...
    try {
      ...
      const { r: k, s: w } = R,
        ...
        _ = U.create(P * v),
        y = U.create(k * v),
        u = $.BASE.multiplyUnsafe(_).add(g.multiplyUnsafe(y))
      if (u.is0()) return !1
  ...
  function T(M, H, Z = {}) {
    ...
    return (
      (H = G0(H, D)), b.fromBytes(M, 'recovered').recoverPublicKey(H).toBytes()
    )
  }
  return Object.freeze({
    keygen: V,
...
var iQ = ($) => {
  ...
  t8 = ($) => {
    const { Fn: J, BASE: q } = m0,
      Q = L0(J, $),
      G = q.multiply(Q)
    return { scalar: $$(G.y) ? Q : J.neg(Q), bytes: e8(G) }
  },
  p$ = ($) => {
    const J = H8
  ...
  i$ = (...$) => m0.Fn.create(e0(O8('BIP0340/challenge', ...$))),
  o$ = ($) => t8($).bytes,
  aQ = ($, J, q = P0(32)) => {
    const { Fn: Q } = m0,
      G = m('message', $),
      { bytes: z, scalar: U } = t8(J),
      Y = m('auxRand', q, 32),
      X = Q.toBytes(U ^ e0(O8('BIP0340/aux', Y))),
      V = O8('BIP0340/nonce', X, z, G),
      { bytes: N, scalar: L } = t8(V),
      j = i$(N, z, G),
      K = new Uint8Array(64)
    if ((K.set(N, 0), K.set(Q.toBytes(Q.create(L + j * U)), 32), !a$(K, G, z)))
...
var u0 = {
  ...
  s8 = BigInt(2),
  H8 = T0(u0.p, { sqrt: iQ }),
  r$ = l$({ ...u0, Fp: H8, lowS: !0, endo: rQ }, M8),
  d$ = {},
  ...
  x8 = (() => {
    ...
    function Q(G) {
      const z = q(G)
      return { secretKey: z, publicKey: o$(z) }
    }
    return {
      keygen: Q,
      getPublicKey: o$,
...
var Q$ = 'AES-GCM',
  ...
  t$ = async ($, J, q) => {
    ...
    return crypto.subtle.importKey('raw', Q, { name: Q$ }, !1, [
      'encrypt',
      'decrypt'
    ])
  },
  e$ = async ($, J) => {
    const q = crypto.getRandomValues(new Uint8Array(12)),
      Q = await $,
      G = await crypto.subtle.encrypt({ name: Q$, iv: q }, Q, j0(J))
  ...
  $Q = async ($, J) => {
    ...
    const G = Uint8Array.from(q.split(s$), Number),
      z = await $,
      U = await crypto.subtle.decrypt({ name: Q$, iv: G }, z, $J(Q))
    return f0(U)
  }
var r = 'GenosRTC',
...
var Q8 = ($) =>
  ...
  { entries: JQ, fromEntries: R8, keys: J1 } = Object,
  qQ = () => {},
...
var QJ = 5000,
  ...
  JJ = 'answer',
  qJ = [
    ...F0(3, ($, J) => `stun:stun${J || ''}.l.google.com:19302`),
    'stun:stun.cloudflare.com:3478'
  ].map(($) => ({ urls: $ })),
  A8 = ($, { rtcConfig: J, rtcPolyfill: q, turnConfig: Q }) => {
...
var GJ = Object.getPrototypeOf(Uint8Array),
  ...
  XQ = ($, J, q) => {
    const Q = new Map(),
      ...
      q0 = (Z, D) => {
        try {
          ...
          const t = new Uint8Array(
            F.chunks.reduce((p, d) => p + d.byteLength, 0)
          )
          F.chunks.reduce((p, d) => (t.set(d, p), p + d.byteLength), 0),
            delete u[B]
          let z0
          if (v) z0 = t
          else if (_)
      ...
      { send: n, on: c } = f(h0('ping')),
      { send: s, on: C0 } = f(h0('pong')),
      { send: G0, on: I } = f(h0('signal')),
      { send: X0, on: S } = f(h0('stream')),
      { send: W, on: T } = f(h0('track')),
      { send: M, on: H } = f(h0('leave'))
    if (
      ($((Z, D) => {
        if (Q.has(D)) return
        Q.set(D, Z),
...
var jJ = 20,
  XJ = 5333,
  YQ = 57333,
  ZQ = ({ init: $, subscribe: J, announce: q }) => {
    const Q = {}
    let G = !1,
      z,
      U
    return (Y, X, V) => {
      ...
      const j = {},
        ...
        E = $8(J8(x, N0)),
        f = t$(Y.password || '', N, X),
        q0 = (S) => async (W) => ({ type: W.type, sdp: await S(f, W.sdp) }),
        b = q0($Q),
        n = q0(e$),
        c = (S, W) => {
          if (K[W] === S) return
        ...
        C0 = (S) => async (W, T, M) => {
          ...
          if (C) {
            ...
            const B = A8(!1, Y)
            B.setHandlers({ connect: () => c(B, D), close: () => s(B, D) })
            try {
              const R = await b(C)
              if (B.isDead) return
            ...
            const B = U.pop() || A8(!0, Y),
              [R, { offer: g }] = await V0([
                $8(J8(x, D)),
                B.offerPromise.then(n).then((k) => ({ offer: k }))
              ])
            ;(j[D] ||= []),
              (j[D][S] = B),
              B.setHandlers({ connect: () => c(B, D), close: () => s(B, D) }),
              M(R, e({ peerId: N0, offer: g, peer: B }))
          }
        },
...
async function RQ() {
  try {
    const $ = JSON.parse(localStorage.getItem(U8) ?? '[]'),
      J = await fetch('https://api.nostr.watch/v1/online')
...
var KJ = ($, J, q) => {
          ...
          if (Y !== U$) {
            if (Y === 'NOTICE' || (Y === 'OK' && !V)) {
              if ((Y === 'NOTICE' ? X : N).match(HQ)?.[1] > 0)
                E8.add(G.replace(/\/$/, '')), z.close?.(), delete c0[G]
            }
            return
          }
          l0[X]?.(v8[X], V.content)
  ...
  WQ = async ($, J) => {
    const q = {
      ...
      G = x8.sign(Q, OQ)
    return e([U$, { ...q, id: T8(Q), sig: T8(G) }])
  },
  NQ = ($, J) => {
    return (
      (v8[$] = J),
      e(['REQ', $, { kinds: [TQ(J)], since: xQ(), ['#' + BQ]: [J] }])
    )
  },
  VQ = ($) => {
          ...
          if (X !== U$) {
            if (X === 'NOTICE' || (X === 'OK' && !N)) {
              if ((X === 'NOTICE' ? V : L).match(HQ)?.[1] > 0)
                E8.add(z.replace(/\/$/, '')), U.close?.(), delete c0[z]
            }
            return
          }
          l0[V]?.(v8[V], N.content)
